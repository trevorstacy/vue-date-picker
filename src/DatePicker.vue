    ...
    <div class="calendar-container">
      <div class="calendar">
        <div :style="{ 'background-color': color }" class="calendar-header flex-center">
          <div>
            <h3 :class="{ 'calendar-faint': selecting === 'date' }" @click="setSelecting('year')">
              {{ selectedYear }}
            </h3>

            <h2 :class="{ 'calendar-faint': selecting === 'year' }" @click="setSelecting('date')">
        ...
        <div class="calendar-body" v-if="selecting === 'date'">
          <div class="calendar-date">
            <div>
              ...
              <div class="calendar-arrows flex left" @click="setByMonth(currentMonth - 1)" v-if="showLeftArrow">
            ...
            <div>
              <div class="calendar-arrows flex right" @click="setByMonth(currentMonth + 1)" v-if="showRightArrow">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
            ...
            <tbody>
              <tr v-for="days in calendar">
                <td :style="{
                        ...
                        }" v-for="day in days" tabindex="0" @keydown.enter="onInput" @keydown.space.stop.prevent="onInput" @keydown.esc="onClose" @click="setByDay(day)">{{ day.day }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        ...
        <div class="calendar-year-select" v-if="selecting === 'year'">
          <div :style="{ 'color': year.selected ? color : '' }" :class="{ 'selected': year.selected }" :id="`${year.year}-calendar-year`" v-for="year in years" @click="setByYear(year.year)">
            {{ year.year }}
          </div>
        </div>
